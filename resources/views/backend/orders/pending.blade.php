                ...
                <div class="row">
                    <div class="col-12">
                        <div class="margin" style="margin-bottom: 10px;">
                            <div class="btn-group">
                                <a href="{{ route('orders.pending') }}" class="btn btn-info btn-block"><i class="fa fa-refresh"></i> Reload</a>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">All Pending Order Details</h3>
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                    @if(\App\Orders::WhereNull('is_deleted')->where('status','Pending')->get()->count()>0)
                                        @foreach(\App\Orders::WhereNull('is_deleted')->where('status','Pending')->orderby('created_at','DESC')->get() as $row)
                                            <tr>
                                                <td>{{ $loop->index+1 }}</td>
                                                <td>{{ $row->id }}</td>
                                                ...
                                                <td>{{ isset($row->payment->created_at)?  date('d, M Y, h:i:s A', strtotime($row->payment->created_at)) : '' }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('orders.view', [HashEncode($row->id), 'order_pending']) }}" class="btn btn-success"><i class="fa fa-eye"></i></a>
