...
@section('page_title','Confirmed Reservations')
                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">All Confirmed Reservations</h3>
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                    @if(\App\Service_booking::whereNull('is_deleted')->where('status','Confirmed')->get()->count()>0)
                                        @foreach(\App\Service_booking::whereNull('is_deleted')->where('status','Confirmed')->orderby('created_at','DESC')->get() as $row)
                                            <tr>
                                                ...
                                                <td>{{ date('D', strtotime($row->confirmed_date)) }} {{ date('Y-m-d h:i:s A', strtotime($row->confirmed_date)) }}</td>
                                                <td>{{ date('D', strtotime($row->payment->created_at)) }} {{ date('Y-m-d h:i:s A', strtotime($row->payment->created_at)) }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('reservations.confirmed.show', HashEncode($row->id))}}" class="btn btn-success"><i class="fa fa-eye"></i></a>
