...
@section('page_title','Completed Reservations')
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="margin" style="margin-bottom: 10px;">
                            <div class="btn-group">
                                <a href="{{ route('reservations.completed') }}" class="btn btn-info btn-block"><i class="fa fa-refresh"></i> Reload</a>
                            </div>
                            {{--<div class="btn-group">--}}
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">All Completed Reservations</h3>
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                    @if(\App\Service_booking::whereNull('is_deleted')->where('status','Completed')->get()->count()>0)
                                        @foreach(\App\Service_booking::whereNull('is_deleted')->where('status','Completed')->orderby('created_at','DESC')->get() as $row)
                                            <tr>
                                                ...
                                                <td>{{ date('D', strtotime($row->updated_at)) }} {{ date('Y-m-d h:i:s A', strtotime($row->updated_at)) }}</td>
                                                <td>{{ date('D', strtotime($row->created_at)) }} {{ date('Y-m-d h:i:s A', strtotime($row->payment->created_at)) }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('reservations.completed.show', HashEncode($row->id))}}" class="btn btn-success"><i class="fa fa-eye"></i></a>
