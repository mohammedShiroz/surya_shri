...
@section('body_content')
                    ...
                    <div class="col-md-12 col-sm-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{ isset($edit_enabled)? 'Edit '.$data->name : 'Add a ' }} service category </h3>
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('service-category.store') }}" method="post" class="form-horizontal" role="form">
                                    @csrf
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea class="form-control @error('description') is-invalid @enderror" row="5" name="description" placeholder="Description"></textarea>
                                        </div>
                                    </div>
                                    @else
                                    <form action="{{ route('service-category.update',$data->id) }}" method="POST" class="form-horizontal add_form" role="form">
                                        @csrf
                                        @method('PUT')
                                        <div class="form-group row">
                                            <div class="col-sm-10 mb-3">
                                        ...
                                        <div class="form-group row">
                                            <label for="inputEmail3" class="col-sm-2 col-form-label">Description</label>
                                            <div class="col-sm-10">
                                                <textarea class="form-control @error('description') is-invalid @enderror" row="5" name="description" placeholder="Description">{{ (old('description'))? old('description'): $data->description }}</textarea>
                                            </div>
                                        </div>
                                @endif
                            </div>
                            ...
                            <div class="card-footer">
                                ...
                                <a href="{{ route('service-category.index') }}" style="margin-right:5px;" class="btn btn-danger float-right">Cancel</a>
                                </form>
                            </div>
                            <!-- /.card-footer -->
                        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="margin" style="margin-bottom: 10px;">
                            @if(isset($edit_enabled))
                            <div class="btn-group">
                                <a href="{{ route('service-category.index') }}" class="btn btn-success btn-block"><i class="fa fa-plus"></i> Add Category</a>
                            </div>
                            @endif
                            <div class="btn-group">
                                <a href="{{ route('service-category.index') }}" class="btn btn-info btn-block"><i class="fa fa-refresh"></i> Reload</a>
                            </div>
                            {{--<div class="btn-group">--}}
                                ...
                                {{--<form method="POST" id="delete-all-form"--}}
                                      {{--action="{{route('service-category.destroy_all')}}">--}}
                                    {{--@csrf--}}
                                {{--</form>--}}
                            {{--</div>--}}
                        </div>
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                    @if(\App\Service_category::all()->count()>0)
                                        @foreach(\App\Service_category::orderby('created_at','desc')->get() as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('service-category.show', $row->id)}}" class="btn btn-success"><i class="fa fa-eye"></i></a>
                                                        @permission('update-service-category')
                                                        <a href="{{ route('service-category.edit', $row->id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a>
                                                        @endpermission
                                                        ...
                                                        {{--<form method="POST" id="{{'delete-form-'.$row->id}}"--}}
                                                              {{--action="{{route('service-category.destroy',$row->id)}}">--}}
                                                            {{--@csrf--}}
                                                            {{--@method('DELETE')--}}
                                                        {{--</form>--}}
                                                    </div>
    ...
    <div class="modal fade" id="modal-bulk-delete">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-info">
                    <h4 class="modal-title">Delete All Service Category</h4>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>Are sure want to delete all service categories?</p>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-info" data-dismiss="modal">Close</button>
    ...
    <form action="{{ route('admin.record.delete','service-categories') }}" id="delete-record-form" method="POST"> @csrf</form>
@endsection
