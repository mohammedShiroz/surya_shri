                ...
                <div class="row">
                                ...
                                <form class="form-horizontal" action="{{ route('doctors.store') }}" method="POST">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <select class="form-control select2 @error('partner_id') is-invalid @enderror" name="partner_id" style="width: 100%;">
                                                <option disabled selected>Select Partner to add as a Doctor</option>
                                                @foreach(\App\Agent::WHERENULL('is_doctor')->ORDERBY('created_at','DESC')->get() as $row)
                                                    <option value="{{ $row->id }}" {{ (old('partner_id'))? (($row->id == old('partner_id'))? 'selected': '') : '' }}>({{ $row->id }}) - {{ $row->user->name." ".$row->user->last_name[0] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="margin" style="margin-bottom: 10px;">
                            <div class="btn-group">
                                <a href="{{ route('doctors.index') }}" class="btn btn-info btn-block"><i class="fa fa-refresh"></i> Reload</a>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @if(\App\Agent::whereNotNull('is_doctor')->get()->count()>0)
                                        @foreach(\App\Agent::whereNotNull('is_doctor')->orderBy("is_doctor", 'desc')->get() as $row)
                                            <tr>
                                                <td>{{ $loop->index+1 }}</td>
                                                ...
                                                <td><span class="badge badge-{{ ($row->is_doctor && $row->is_seller)? 'success' : (($row->is_doctor)? 'warning text-white': 'warning') }}">{{ ($row->is_doctor && $row->is_seller)? 'Doctor & Seller' : (($row->is_doctor)? 'Doctor': 'Seller') }}</span></td>
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($row->is_doctor)
                                                        {{ $row->services->count() }} Service
                                                    @endif
                                                </td>
                                                <td>{{ date('Y-m-d h:i:s A', strtotime(($row->is_seller)? $row->is_seller : $row->is_doctor)) }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('doctors.show', $row->user->id) }}" class="btn btn-success btn-sm"><i class="fa fa-eye"></i> Details</a>
