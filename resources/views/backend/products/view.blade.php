...
@section('page_title','View Product Details')
                            ...
                            <div class="card card-widget widget-user">
                                ...
                                <div class="card-footer" style="padding-top: 100px">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-4 border-right">
                                            <div class="description-block">
                                                <h5 class="description-header">{{ ($data->status == "PUBLISHED")? 'PUBLISHED' : 'UNPUBLISHED' }} </h5>
                        ...
                        <div class="col-sm-12 col-md-7">
                            ...
                            <div class="card bg-light">
                                ...
                                <div class="card-body pt-0">
                                    <div class="row">
                                        <div class="col-8">
                                            <h2 class="lead"><b>{{ $data->seller_info->user->name }}</b></h2>
                                            <p class="text-muted text-sm" style="margin-top: -10px;">({{ ($data->seller_info->is_seller && $data->seller_info->is_doctor)? ' Doctor & Seller ' : (($data->seller_info->is_doctor)? 'Doctor' : 'Seller') }})</p>
                                            <ul class="ml-4 mb-0 fa-ul text-muted">
                                                <li class="small"><span class="fa-li"><i class="fas fa-envelope"></i></span> Email: {{ $data->seller_info->user->email }}</li>
                                                <li class="small mt-1"><span class="fa-li"><i class="fas fa-phone"></i></span> Phone: {{ $data->seller_info->user->contact }}</li>
                                            </ul>
                                        </div>
                                        <div class="col-4 text-center">
                                            <img src="{{ asset($data->seller_info->user->profile_image) }}" alt="user-avatar" width="100px" height="100px" class="img-circle img-fluid">
                                        </div>
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <div class="d-inline-block ">
                                        ...
                                        <input type="hidden" id="social_share_url" value="{{ route('home')."/products/".$data->slug }}"/>
                                    ...
                                    <div class="d-inline-block float-right pt-2">
                                        <a href="{{ route('sellers.show', $data->seller_info->user->id) }}" class="btn btn-info">
                            ...
                            <div class="bg-gray py-2 px-3 mt-4">
                                ...
                                <h4 class="mt-0 text-md">
                                    ...
                                    <small>{!! ($data->seller_paid)? 'Seller Paid: '.$data->seller_paid."%<br/>Seller Paid Amount: ".getCurrencyFormat($data->seller_paid_amount) : '' !!}</small>
                                </h4>
                            </div>
                        ...
                        <div class="col-md-12 col-sm-12">
                            <nav class="w-100">
                                <div class="nav nav-tabs" id="product-tab" role="tablist">
                                    <a class="nav-item nav-link {{ (session('active_tab') && session()->get("active_tab")=='reviews')? '' : 'active' }}" id="product-info-tab" data-toggle="tab" href="#product-info" role="tab" aria-controls="product-info" aria-selected="true">Item Details</a>
                            ...
                            <div class="tab-content p-3" id="nav-tabContent" style="width: 100%; padding: 0 !important;">
                                <div class="tab-pane fade {{ (session('active_tab') && session()->get("active_tab")=='reviews')? '' : 'show active' }}"  id="product-info" role="tabpanel" aria-labelledby="product-info-tab">
                                    <table class="table border mt-2">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(isset($data->category->parent->parent->name))
                                                    {!!  isset($data->category->parent->parent->name)? $data->category->parent->parent->name.' <strong><i class="fa fa-angle-double-right text-sm"></i></strong>': '' !!}
                                                @endif
                                                @if(isset($data->category->parent->name))
                                                    {{ $data->category->parent->name }} <strong><i class="fa fa-angle-double-right text-sm"></i></strong>
                                                @endif
                                                {{ $data->category->name }}
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td>Pending Order Qty</td>
                                            <td>{{ get_pending_stock($data->id) }}</td>
                                        </tr>
                                        <tr>
                                            <td>Sold</td>
                                            <td>{{ $data->sold }}</td>
                                                    ...
                                                    <td>
                                                        <div class="rating_wrap">
                                                            ...
                                                            <span class="rating_num">
                                                                <i class="ti-comment"></i> ({{ $data->reviews->count() }})
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
                                                ...
                                                @foreach($data->reviews as $row)
                                                    <div class="media border mt-0 p-3 mb-3">
                                                        ...
                                                        <div class="media-body">
                                                            ...
                                                            <form action="{{ route('products.remove.user.review') }}" method="POST" class="d-inline-block">
                                                                @csrf
                                                            ...
                                                            <div class="modal fade" id="Comment-{{$row->id}}" tabindex="-1" role="dialog"  aria-hidden="true">
                                                                <div class="modal-dialog" role="document">
                                                                    <form action="{{ route('products.add.reply') }}" method="post">
                                                                        @csrf
                                                                    ...
                                                                    <div class="media mt-4">
                                                                        ...
                                                                        <div class="media-body">
                                                                            <h5 class="mt-0 mb-1 font-weight-semibold">Suryashri.lk <span class="fs-14 ml-0">
                                                                                        <form action="{{ route('products.remove.admin.reply') }}" method="POST" class="d-inline-block">
                                                                                            @csrf
                                                                                            <input type="hidden" name="active_tab" value="reviews">
