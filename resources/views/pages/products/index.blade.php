...
@if(isset($old_search_word))
    @section('page_title','Search Boutique')
@else
    @if(!request()->routeIs('wish-list'))
...
@section('body_content')
    @if(isset($old_search_word))
        @include('components.breadcrumb',['page_title_text'=>'Search Boutique','paths'=>[
            0=>['name'=>'<i class="fa fa-home fa-sm"></i>Home','route'=>route('home')],
            1=>['name'=>'Products','route'=>route('products')],
            2=>['name'=>'Search result','route'=>''],
        ]])
    @else
        @if(!request()->routeIs('wish-list'))
            @include('components.breadcrumb',['page_title_text'=>'Boutique','paths'=>[
                0=>['name'=>'<i class="fa fa-home fa-sm"></i>Home','route'=>route('home')],
                1=>['name'=>'Boutique','route'=>''],
            ]])
        @else
            @include('components.breadcrumb',['page_title_text'=>'Wish List','paths'=>[
                0=>['name'=>'<i class="fa fa-home fa-sm"></i>Home','route'=>route('home')],
                1=>['name'=>'Boutique','route'=>route('products')],
                2=>['name'=>'wish-list','route'=>''],
            ]])
        @endif
    @endif
    @php $is_wish_list_available=false; if(request()->routeIs('wish-list')){ $is_wish_list_available=true;} @endphp
    <!-- START SECTION SHOP -->
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-9">
                                    ...
                                    <div class="products_view">
                                        @if($is_wish_list_available == true)
                                            @if(\App\Wishlist::where('user_id', \Auth::user()->id)->get()->count() > 0)
                                                <a href="javascript:Void(0);" class="shorting_icon-delete" id="clear_all_wish_list_products"><i class="ti-trash"></i> Clear All</a>
                                            @endif
                                        @endif
                    ...
                    @include('components.products.products_data_fetch')
                </div>
                <div class="col-lg-3 order-lg-first mt-4 pt-2 mt-lg-0 pt-lg-0">
                    @include('components.products.category_filter')
                </div>
            </div>
        </div>
    </div>
    ...
    <input type="hidden" id="second_lvl_tag" value="{{ isset($second_lvl_category_detail)? $second_lvl_category_detail->id : '' }}" />
    <input type="hidden" id="third_lvl_tag" value="{{ isset($three_lvl_category_detail)? $three_lvl_category_detail->id : '' }}" />
    <input type="hidden" id="wish_list_check" value="{{ ($is_wish_list_available == true)? '1':'0' }}" />
    <input type="hidden" id="search_check" value="{{ isset($old_search_word)? $old_search_word : '' }}" />
@endsection
@push('js')
...
<script>
    $(document).ready(function () {
        ...
        if($("#second_lvl_tag").val()){
            var cty_id=$("#second_lvl_tag").val();
            $("#second_tg_"+cty_id).trigger('click');
            $("#second_tg_"+cty_id).prop('checked', true);
        }
        //:If other sub category has in the url
        if($("#third_lvl_tag").val()){
            var cty_id=$("#third_lvl_tag").val();
            $("#third_tg_"+cty_id).trigger('click');
            $("#third_tg_"+cty_id).prop('checked', true);
        }
        ...
        $(document).on('click', '.common_category_selector', function(event){
            filter_data();
        });
        function filter_data() {
            ...
            var stock = $("#available_check").val();
            var show_item =parseInt($("#get_showing_number").val());
            var sorting= $("#get_sorting_value").val();
            var page=$("#hidden_page").val();
            var first_lvl_tag= $("#first_lvl_tag").val();
            var second_lvl_tag = get_filter('search-category');
            var third_lvl_tag =get_filter('search-sub-category');
            $.ajax({
                url:"/fetch-data/product-data?page="+page+"&minimum_price="+minimum_price+"&maximum_price="+maximum_price+"&stock="+stock+"&show_item="+show_item+"&sorting="+sorting+"&first_lvl_tag="+first_lvl_tag+"&second_lvl_tag="+second_lvl_tag+"&third_lvl_tag="+third_lvl_tag+"&wish_list_check="+$("#wish_list_check").val(),
                method: "POST",
                data:{},
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function (data) {
        ...
        $(".search-category").on('click', function(){
                if(get_filter('search-category').length >0){
                    $.ajax({
                        url:"/fetch-data/sub_categories",
                        method: "POST",
                        data:{  ids:get_filter('search-category')},
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        success: function (data) {
                            //alert(data);
                            //console.log(data);
                            $('.loading_pr').remove();
                            $('.fetch_third_level_categories_data').empty().html(data);
                        }
                    });
                }else{
                    $('.fetch_third_level_categories_data').empty().html('');
                }
            });
